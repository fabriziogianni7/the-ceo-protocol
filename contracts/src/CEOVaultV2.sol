...
contract CEOVaultV2 is ICEOVaultV2, ERC4626Fees, ReentrancyGuard, Pausable {
    ...
    // ── Approvable Tokens ──
    mapping(address => bool) public s_isApprovableToken;

    // ── Value Adapters (lending, DEX, etc.) ──
    address[] public s_valueAdapters;
    mapping(address => bool) public s_isValueAdapter;

    // ── Selector Whitelist (per whitelisted target) ──
    mapping(address => mapping(bytes4 => bool)) public s_allowedSelectors;
    mapping(address => bytes4[]) private _s_targetSelectors; // for replace semantics
    ...
    /// @notice Add a token to the approvable list (owner or governance via proposal)
    /// @dev Tokens in this list can be approved to whitelisted spenders in action proposals.
    ///      Callable by owner directly, or by governance when included as a self-call in a winning proposal.
    /// @param token Token address to add
    function addApprovableToken(address token) external {
        if (msg.sender != s_owner && msg.sender != address(this)) revert NotOwner();
        if (token == address(0)) revert ZeroAddress();
        if (s_isApprovableToken[token]) return;
        s_isApprovableToken[token] = true;
        emit ApprovableTokenAdded(token);
    }

    /// @notice Remove a token from the approvable list (owner or governance via proposal)
    /// @param token Token address to remove
    function removeApprovableToken(address token) external {
        if (msg.sender != s_owner && msg.sender != address(this)) revert NotOwner();
        if (!s_isApprovableToken[token]) return;
    ...
    /// @dev Adapter must implement IValueAdapter.getDeployedValue(vault) returning USDC-denominated value.
    /// @param adapter Address of the value adapter contract
    function addValueAdapter(address adapter) external onlyOwner {
        if (adapter == address(0)) revert ZeroAddress();
        if (s_isValueAdapter[adapter]) revert ValueAdapterAlreadyAdded();
        if (s_valueAdapters.length >= MAX_VALUE_ADAPTERS) revert TooManyValueAdapters();

        s_valueAdapters.push(adapter);
        s_isValueAdapter[adapter] = true;
        emit ValueAdapterAdded(adapter);
    }

    /// @notice Remove a value adapter
    /// @param adapter Address of the value adapter to remove
    ...
    /// @dev Only explicitly allowed selectors can be called on whitelisted targets.
    /// @param target Whitelisted target address
    /// @param selector Function selector (bytes4)
    function addAllowedSelector(address target, bytes4 selector) external onlyOwner {
        if (target == address(0)) revert ZeroAddress();
        if (!s_allowedSelectors[target][selector]) {
            s_allowedSelectors[target][selector] = true;
            _s_targetSelectors[target].push(selector);
        }
        emit AllowedSelectorSet(target, selector, true);
    }

    /// @notice Remove an allowed selector for a whitelisted target
    /// @param target Whitelisted target address
    /// @param selector Function selector (bytes4)
    ...
    /// @notice Set all allowed selectors for a target (replaces existing)
    /// @param target Whitelisted target address
    /// @param selectors Array of allowed selectors. Empty array = no selectors allowed.
    function setAllowedSelectors(address target, bytes4[] calldata selectors) external onlyOwner {
        if (target == address(0)) revert ZeroAddress();
        // Clear existing
        bytes4[] storage arr = _s_targetSelectors[target];
        for (uint256 i = 0; i < arr.length; i++) {
            s_allowedSelectors[target][arr[i]] = false;
        }
        delete _s_targetSelectors[target];
        for (uint256 i = 0; i < selectors.length; i++) {
            s_allowedSelectors[target][selectors[i]] = true;
            _s_targetSelectors[target].push(selectors[i]);
            emit AllowedSelectorSet(target, selectors[i], true);
        }
    }
    ...
    function executeRebalance(AllocationTarget[] calldata allocations) external {
        ...
        for (uint256 i = 0; i < plan.depositVaults.length; i++) {
            IERC20(asset()).forceApprove(plan.depositVaults[i], plan.depositAmounts[i]);
            try IERC4626(plan.depositVaults[i]).deposit(plan.depositAmounts[i], address(this)) {
                // Success
            } catch {
                IERC20(asset()).forceApprove(plan.depositVaults[i], 0);
                revert ActionFailed();
            }
            IERC20(asset()).forceApprove(plan.depositVaults[i], 0);
        }
    ...
    /// @notice Submit a proposal (ordered list of actions) for the current epoch
    /// @dev Each agent may propose at most once per epoch. Max 10 proposals per epoch.
    ///      Rebalance is an action: target=this, data=executeRebalance(allocations).
    /// @param actions Ordered list of actions to execute when proposal wins
    ...
    /// @notice Get the list of value adapters (lending, DEX, etc.)
    function getValueAdapters() external view returns (address[] memory) {
        return s_valueAdapters;
    }

    /// @notice Get allowed selectors for a whitelisted target
    /// @param target Whitelisted target address
    /// @return selectors Array of allowed function selectors
    function getAllowedSelectors(address target) external view returns (bytes4[] memory) {
        return _s_targetSelectors[target];
    }
    ...
    function validateActions(Action[] calldata actions) external view returns (bool valid) {
        if (actions.length == 0 || actions.length > s_maxActions) return false;
        for (uint256 i = 0; i < actions.length; i++) {
            if (!_validateAction(actions[i])) return false;
        }
        return true;
    }

    /// @notice Simulate action-based proposal execution
    /// @dev Call via eth_call. Runs validation + execution, then reverts with SimulationComplete.
    ///      Decode revert data to get (success, failedActionIndex, revertReason).
    function simulateActions(Action[] calldata actions) external {
        if (actions.length == 0 || actions.length > s_maxActions) {
            revert SimulationComplete(false, 0, abi.encodeWithSignature("TooManyActions()"));
        }

        for (uint256 i = 0; i < actions.length; i++) {
            if (!_validateAction(actions[i])) {
                revert SimulationComplete(false, i, abi.encodeWithSignature("ActionNotAllowed()"));
            }
        }

        for (uint256 i = 0; i < actions.length; i++) {
            (bool ok, bytes memory reason) = actions[i].target.call(actions[i].data);
            if (!ok) {
                revert SimulationComplete(false, i, reason);
            }
        }

        _revokeTokenApprovals(actions);

        revert SimulationComplete(true, 0, "");
    }
    ...
    /// @dev Check drawdown invariant after execution
    function _checkDrawdown(uint256 totalBefore) internal view {
    ...
    /// @dev Mark proposal as executed and update proposer score
    function _markProposalExecuted(Proposal storage p) internal {
        p.executed = true;
    ...
    function _revokeTokenApprovals(Action[] calldata actions) internal {
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            // approve() actions
            bool isApprovable =
                action.target == asset() || action.target == address(i_ceoToken) || s_isApprovableToken[action.target];
            if (!isApprovable) continue;
