...
/// @title ICEOVaultV2 — Interface for The CEO Protocol v2 Vault
...
///         Unified proposals: ordered actions only (rebalance via executeRebalance self-call).
interface ICEOVaultV2 {
    // ══════════════════════════════════════════════════════════════
    //                         ERRORS
    // ══════════════════════════════════════════════════════════════
    ...
    /// @notice Thrown when allocation percentages exceed 100%
    error AllocationBpsExceedMax();

    /// @notice Thrown when allocation targets an invalid or non-yield vault
    error InvalidAllocationVault();

    /// @notice Thrown when allocation array is empty or too long
    error InvalidAllocationCount();

    /// @notice Thrown when proposal has no actions
    error EmptyProposal();

    /// @notice Thrown when simulateActions fails at a specific action index
    error SimulationComplete(bool success, uint256 failedActionIndex, bytes revertReason);

    /// @notice Thrown when adding a value adapter that already exists
    error ValueAdapterAlreadyAdded();

    /// @notice Thrown when removing a value adapter that doesn't exist
    error ValueAdapterNotFound();

    /// @notice Thrown when adding a value adapter would exceed MAX_VALUE_ADAPTERS
    error TooManyValueAdapters();

    /// @notice Thrown when action targets a whitelisted protocol with a disallowed selector
    ...
    /// @notice Plan of withdraws/deposits computed from allocation targets
    struct AllocationPlan {
        address[] withdrawVaults;
    ...
    /// @notice Emitted when a token is added to the approvable list
    event ApprovableTokenAdded(address indexed token);

    /// @notice Emitted when a token is removed from the approvable list
    event ApprovableTokenRemoved(address indexed token);

    /// @notice Emitted when a value adapter is added
    event ValueAdapterAdded(address indexed adapter);

    /// @notice Emitted when a value adapter is removed
